...
public:
	...
	public:
		...
		float ticksPerSecond=59.7*FRAME_DIVIDER;
		uint16_t patLen=64;
...
void songData2fur(std::vector<gb_chip_state> songData, uint16_t inPatLen, bool disablePanMute){
...
#endif
std::string gbsplayCmd = progPrefix+"gbsplay"+progSuffix+" -t "+(argc >= 5 ? std::string(argv[4]) : "150")+" -o iodumper -- "+std::string(argv[1])+" "+std::string(argv[2])+" "+std::string(argv[2]);
...
while (fgets(line, sizeof(line), gbsplayFile)){ 
	...
	uint64_t curFramesPassed = cyclesPassed / CYCLES_PER_FRAME;
...
songData2fur(songData, patternLength, disablePanMute);
